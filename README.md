...
历史任务在启动时**归位**：带项目且有项目群的一律归该项目群（含早期被错写成大厅的任务）；
无项目的归大厅。所以升级后老任务的汇报也会回到正确的项目群。
...
实测事故：董事会只安排了几个任务，公司一天烧到 **¥19.51**（平常 ¥3–5）、
7324 次调用、非缓存输入 77.9M——原因是**链式续派**：任务完成 → 汇报回投唤醒 CEO →
它认为"还有下一批" → 再派一条 → 循环（顾砚一人被派了 18 条「下一刀 / 下一批 / 下批 / 队列批」）。

| 闸门 | 规则 | 效果 |
|---|---|---|
| **派活来源** | agent 只能在**董事会触发的轮次**里派活。汇报回投 / 排程 / 转投触发的轮次调用 `company_dispatch` → 拒绝 `not_board_triggered` | 掐断链式续派 |
| **自我排程** | agent（含 CEO）**不得** `company_schedule_create`；排程只能董事会在面板建 | 掐断自我唤醒链 |
| **任务级预算** | `taskTokenCap`（默认 8M，输入+输出+缓存合计）：单个任务会话超限即停 + 任务留言 | 单任务不再能跑到几亿 |

外加：员工本来就没有派活/建任务权（工具面里不出现 + 服务端拒绝），已在实测中确认有效。
